 ...
 *    3.0   : modify the structure of winch drive, the scan frequency of screw drive
 *            for new motor drive frequency
*/
//------------------------------------------------------------------------------
// Libraries
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
const int SLEEP_TIME_IN = 50;               // Scan frequency for Tension Sensor input (ms)
...
const int SLEEP_TIME_OPT_WINCH = 500;       // Scan frequency for Winch Motor Output Pulse Generator (us)
...
const int MIN_ALLOW_STEP = -10000;          // Minimum allowed step (end of screw)
const float MAX_SET_TENSION = 350.0;        // Maximum allowed loading target
...
// for close loop simulation only - to be removed in production
static int sim_last_step;
static float sim_step_out;
...
static void thTensionIn(void *arg){
  while (!chThdShouldTerminateX()) {
     ...
     */
    // -------------------------------------------------simulation start --------------------------------------------
    /* assumption 1: when screw move 1 step towards far end, tension increase by 50;
     *               when screw move 1 step towards far end, tension decrease by 50
     * assumption 2: when winch move direct (release) 1 step, tension decrease by 2;
     *               when winch move reverse (release) 1 step, tension increase by 2
     */
      flTension = flTension + 50.0 * (float)(intStepCount-sim_last_step) + sim_step_out;
      sim_last_step = intStepCount;
      sim_step_out = 0.0;
    //--------------------------------------------------simulation end ----------------------------------------------
...
static void thWinchCtrl(void *arg){
    ...
    /* assumption 1: when screw move 1 step towards far end, tension increase by 50;
     *               when screw move 1 step towards far end, tension decrease by 50
     * assumption 2: when winch move direct (release) 1 step, tension decrease by 0.25;
     *               when winch move reverse (release) 1 step, tension increase by 0.25
     */
      if (boolStepDirection){
        sim_step_out = sim_step_out - 0.25 * (float)intStepOutput; 
      }else{
        sim_step_out = sim_step_out + 0.25 * (float)intStepOutput;
      }
    //--------------------------------------------------simulation end ----------------------------------------------
...
//------------------------------------------------------------------------------
// Process - Winch Motor Output Pulse Generator
//------------------------------------------------------------------------------
static THD_WORKING_AREA(waWMO,128);
static void thWinchOpt(void *arg){
  while (!chThdShouldTerminateX()) {
    if (intStepOutput > 0){
        ...
        delayMicroseconds(2);
        digitalWrite(52,LOW);
        intStepOutput = intStepOutput -1;
    }
    chThdSleepMicroseconds(SLEEP_TIME_OPT_WINCH);
  }
}
//------------------------------------------------------------------------------
...
void chMain() {  
      ...
      chThdCreateStatic(waWMO, sizeof(waWMC),NORMALPRIO, thWinchOpt, NULL);
...
void setup() {
  ...
  // for simulation only
      sim_step_out = 0;
      sim_last_step = 0.0;
  // remove above statements in production
